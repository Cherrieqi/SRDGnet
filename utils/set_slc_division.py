...
def set_shuffle(image_slice, label_slice):
    """
    :param image_slice: 高光谱图像切片 [N, c, slice_size, slice_size]
    :param label_slice: 真值
    :return: image_slice_shuffle, label_slice_shuffle : 打乱后的图像切片和真值
    """
    N = image_slice.shape[0]  # 样本数量
    ...
    idx = list(range(N))
    random.seed(1234)
    random.shuffle(idx)  # idx为打乱后样本的索引
